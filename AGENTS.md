...
- `spikes/weekly_all.py` — multi-topic catch-up runner (`make spike-weekly-all`): regenerates every topic whose last selection isn't fresh (no picks within `RECENCY_DAYS`), 3 topics at a time (`WEEKLY_WORKERS`), one shared rate limiter, failures isolated. Freshness source: per-topic picks `spikes/state/picks/{slug}.json` (legacy single-file fallback, materialized on first pass).
