...
- **Multi-topic iteration**: the weekly stage iterates every topic in `signalflow/topics.json` that has a `docs/discovery/{slug}.json`; topics without a discovery list get one generated on the fly (the `make topic-sources` flow: generate → gates → review until approval; a needs-human list is still persisted and used, reported for later review), then selection runs on it — no manual step. Per-topic picks aggregate into one digest. Spike implementation (`spikes/weekly_all.py`, `make spike-weekly-all`): regenerates only topics whose last selection is NOT fresh (no picks within `RECENCY_DAYS`), `WEEKLY_WORKERS` (3) topics at a time, one shared rate limiter, failures isolated; per-topic picks persist to `spikes/state/picks/{slug}.json` (legacy single-file fallback materialized on first pass).
- **Per-topic feeds (decision 2026-08-03)**: `make feeds` renders ONE RSS feed per topic (`spikes/output/site/feeds/{slug}.xml`) plus the topic's background long-read page (`spikes/output/site/topics/{slug}/index.html`), both statically hosted (hosting = OQ-3). Entry: id = source URL; description = OUR why-it-matters summary (the pick `reason`, falling back to `empirical_event`, then `thesis` — this supersedes "reason dropped" for the per-topic feeds); body = that summary, then a link to the topic long read, then a clickable preview snippet of the article (item summary from the feed cache, truncated) linking to the original.
