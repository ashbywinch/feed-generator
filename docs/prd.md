...
- **Multi-topic iteration**: the weekly stage iterates every topic in `signalflow/topics.json` that has a `docs/discovery/{slug}.json`; topics without a discovery list are skipped and reported (they cannot be fed-selected until `make topic-sources` produces a list). Per-topic picks aggregate into one digest. Spike implementation (`spikes/weekly_all.py`, `make spike-weekly-all`): regenerates only topics whose last selection is NOT fresh (no picks within `RECENCY_DAYS`), `WEEKLY_WORKERS` (3) topics at a time, one shared rate limiter, failures isolated; per-topic picks persist to `spikes/state/picks/{slug}.json` (legacy single-file fallback materialized on first pass).
